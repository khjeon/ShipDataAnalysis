...
def shipDataQuery(callSign, beginDate, EndDate, dataSplitCount, features, isShuffle):
    ...
    stmt = "SELECT" + features + "FROM [SHIP_DB].[dbo].[SAILING_DATA] WHERE CALLSIGN ='"+callSign+"' AND TIME_STAMP > '"  + beginDate + "' AND TIME_STAMP <  '"+ EndDate +"' AND AT_SEA = 1 AND PRIMARY_DATA_CHECK = 1 AND ERROR_MEFOFLOW_DATA = 1 AND ERROR_SHAFTREV_DATA = 1 AND SPEED_VG > 6 AND SPEED_VG < 18 AND BHP_BY_FOC > 1000 AND DRAFT_FORE > 3 AND DRAFT_AFT < 20 ORDER BY TIME_STAMP"
