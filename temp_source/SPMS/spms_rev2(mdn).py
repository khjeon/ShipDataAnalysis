...
class hmdn_class(object):
    ...
    def plot_recon_hmdn(self, GRAPHCOUNT):
        ...
        speedRange = (int(np.max(xdata_inv1[:, 2]) - np.min(xdata_inv1[:, 2]))+1)*10
        print(speedRange )
        speenmin = round(np.min(xdata_inv1[:, 2]), 1)
        ...
        for plot_dim in range(ndim):
            plot3_2(xdata_inv1[:, 4], xdata_inv1[:, 2], ydata_inv2[:, plot_dim], xdata_inv1[:, 4], xdata_inv1[:, 2], pred_ydata_inv2[:, plot_dim], speed, powerBallast, powerLaden, GRAPHCOUNT, l1="Training Data", l2="Predicted Data", title=("[%s] kmix: [%d], dim: [%d], mse_data: [%.d], r2: [%.2f], rmse: [%d]" % (self.name, self.kmix, dim, mse_data, r2, rmse)))
    # SAVE WEIGHTS
...
data_opt = {
    ...
    'endTrainDate' : '2016-11-01',
    'trainDataCount' : 80000000,
    'isTrainDataShuffle' : 0,
    'startTestDate' : '2016-11-01',
    ...
    'kalmanfilter' : 0,
    'normalization' : 1,
    'features' : ['REL_WIND_DIR', 'REL_WIND_SPEED', 'SPEED_VG', 'SHIP_HEADING', 'SHAFT_REV', 'DRAFT_FORE', 'DRAFT_AFT', 'WATER_DEPTH', 'RUDDER_ANGLE', 'SST'],
    'label' : ['BHP_BY_FOC']
}
