...
def DataQuery(callSign, startDate, endDate, dataCount, QueryData, isShuffle):
    ...
    & (_data['SLIP'] > -50) & (_data['SLIP'] < 50) & (_data['DRAFT_FORE'] > 2) & (_data['DRAFT_FORE'] < 30) \
    & (_data['DRAFT_AFT'] > 2) & (_data['DRAFT_AFT'] < 30) & (_data['REL_WIND_DIR'] >= 0) & (_data['REL_WIND_DIR'] <= 360) \
...
data_opt = {
    'callSign' : "3ewb4",
    'startTrainDate' : '2016-09-14',
    'endTrainDate' : '2016-09-24',
    'trainDataCount' : 80000000,
    'isTrainDataShuffle' : 0,
    'startTestDate' : '2016-06-02',
    'endTestDate' : '2017-11-28',
...
plt.plot(data['SPEED_VG'], data['SLIP'], 'ro')
plt.xlabel('SPEED_VG', color='red')
plt.ylabel('SLIP', color='red')
plt.grid(True)
plt.show()
print(data.describe())