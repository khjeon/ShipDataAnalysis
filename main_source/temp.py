...
def pitch(callSign):
    conn = pymssql.connect(server='218.39.195.13:21000', user='sa', password='@120bal@', database='SHIP_DB_EARTH')
    pitchQuery = "SELECT TOP 1 [PROPELLER_PITCH] FROM [SHIP_DB].[dbo].[SHIP_PARTICULAR] WHERE CALLSIGN ='"+callSign+"'"
    result = pd.read_sql(pitchQuery,conn).values
    return result
...
class hmdn_class(object):
    ...
    def plot_recon_hmdn(self, GRAPHCOUNT):
        ...
        slip = 0.034
        x_result_data[:,1] =  x_result_data[:,0] / ((1-slip) * self.pitch/1852/1000 * 60)
        ...
        rmseTrain = sqrt(mean_squared_error(pred_yTestData_inv2, yTestData_inv2))
        rmseTest = sqrt(mean_squared_error(pred_yTrainData_inv2, yTrainData_inv2))
        speed = []
        powerBallast = []
        powerLaden = []
...
def DataQuery(callSign, startDate, endDate, averaging, dataCount, QueryData, isShuffle, features, label):
    ...
    & (_data['SLIP'] > -50) & (_data['SLIP'] < 50) & (_data['DRAFT_FORE'] > 13) & (_data['DRAFT_FORE'] < 30) \
    & (_data['DRAFT_AFT'] > 13) & (_data['DRAFT_AFT'] < 30) & (_data['REL_WIND_DIR'] >= 0) & (_data['REL_WIND_DIR'] <= 360) \
